...
pub fn parse_file(path: &Path) -> ParseResult {
    ...
    let mut nav_lat: Option<f64> = None;
    let mut nav_lon: Option<f64> = None;
    let mut nav_heading: Option<f32> = None;
    ...
    while pos + JSF_HDR_SIZE <= bytes.len() {
        ...
        if msg_type == MSG_SONAR_DATA && msg_size >= SONAR_BODY_MIN {
            ...
            let sonar_lat = safe_le_f32(body, 32) as f64;
            let sonar_lon = safe_le_f32(body, 36) as f64;
            let sonar_heading = safe_le_f32(body, 44);
            let lat = if sonar_lat.abs() > 0.000_001 { sonar_lat } else { nav_lat.unwrap_or(0.0) };
            let lon = if sonar_lon.abs() > 0.000_001 { sonar_lon } else { nav_lon.unwrap_or(0.0) };
            let heading = if sonar_heading.is_finite() && sonar_heading != 0.0 {
                sonar_heading
            } else {
                nav_heading.unwrap_or(0.0)
            };
