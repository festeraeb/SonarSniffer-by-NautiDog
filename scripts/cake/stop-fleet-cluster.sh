...
pkill -f 'cake run worker' 2>/dev/null || true
pkill -f 'cake run master' 2>/dev/null || true
...
if [[ "${FLEET_DISPATCH:-auto}" != "ssh" ]] && [[ -x "${REPO}/scripts/fleet-n8n-dispatch.sh" ]]; then
  ...
  ssh -o ConnectTimeout=5 "${CESAROPS2_USER:-cesarops}@${CESAROPS2_HOST:-10.0.0.201}" \
    "pkill -f 'cake worker.*cluster-key' 2>/dev/null || true; pkill -f 'cake master.*cluster-key' 2>/dev/null || true; pkill -f 'cake run.*cluster-key' 2>/dev/null || true" 2>/dev/null || true
fi
