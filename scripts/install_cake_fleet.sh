...
pick_cuda_compute_cap() {
  if [[ -n "${CUDA_COMPUTE_CAP:-}" ]]; then
    log "CUDA_COMPUTE_CAP=${CUDA_COMPUTE_CAP} (explicit)"
    return 0
  fi
  local cap
  ...
  if [[ "$cap" == "7.5" ]]; then
    ...
    log "Turing (cc $cap) — CUDA_COMPUTE_CAP=75 (native; use install_cake_for_cap.sh + pascal patch)"
  fi
}
...
install_cake_cli() {
  ...
  local -a install_args=(--locked)
  [[ "${CAKE_NO_DEFAULT_FEATURES:-0}" == "1" ]] && install_args+=(--no-default-features)
