...
pub fn detect_and_parse(path: &Path) -> DetectResult {
    ...
    match format {
        ...
        SonarFormat::LowranceSL2 | SonarFormat::LowranceSL3 => {
            ...
            DetectResult { format, parse, probe: placeholder_probe(path) }
        }
...
fn sniff_magic(header: &[u8]) -> SonarFormat {
    if header.len() < 4 { return SonarFormat::Unknown; }
    ...
    if header.len() >= 8 {
        ...
        if fmt == 2 { return SonarFormat::LowranceSL2; }
        if fmt == 3 { return SonarFormat::LowranceSL3; }
    }
    ...
    if header.len() >= 2 {
        ...
        if m == 0xFACE { return SonarFormat::XTF; }
    }

    // JSF start marker 0x1601
    if header.len() >= 2 {
        let m = u16::from_le_bytes([header[0], header[1]]);
        if m == 0x1601 { return SonarFormat::JSF; }
    }
