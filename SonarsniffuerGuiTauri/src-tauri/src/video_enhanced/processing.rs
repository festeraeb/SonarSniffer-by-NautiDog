...
pub fn apply_processing_pipeline(
    ...
    let tvg_lut_side = tvg::precompute_tvg_lut(side_width, params);
...
fn build_intermediate_frame_single(
    ...
    for (row, ping) in pings.iter().enumerate() {
        let start = nadir_skip.min(ping.samples.len());
        let samples = if params.interpolate_gaps {
            interpolate_gaps_u16(&ping.samples[start..], params.gap_threshold_samples)
        } else {
            ping.samples[start..].to_vec()
        };
...
fn build_intermediate_frame_stitched(
    ...
    for (row_idx, src_row) in (row_start..row_end).enumerate() {
        ...
        if src_row < port.len() {
            let raw = &port[src_row].samples;
            let start = nadir_skip.min(raw.len());
            let samples = if params.interpolate_gaps {
                interpolate_gaps_u16(&raw[start..], params.gap_threshold_samples)
            } else {
                raw[start..].to_vec()
            };
            let corrected = tvg::apply_tvg_lut(&samples, tvg_lut);
