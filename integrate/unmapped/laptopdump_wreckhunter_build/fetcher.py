...
# Lake Superior bounding box (includes full lake and Straits of Mackinac)
...
def main():
    ...
    parser.add_argument("--lake", type=str, default="michigan", 
                       choices=["michigan", "superior", "huron", "erie", "ontario", "all"],
                       help="Which Great Lake to download (default: michigan)")
    ...
    # Select lake bounds
    lake_bounds = {
        "michigan": LAKE_MICHIGAN_BOUNDS,
        "superior": LAKE_SUPERIOR_BOUNDS,
        "huron": LAKE_HURON_BOUNDS,
        "erie": {"north": 43.0, "south": 41.0, "west": -83.5, "east": -78.5},
        "ontario": {"north": 44.5, "south": 43.0, "west": -79.5, "east": -76.0},
        "all": ALL_GREAT_LAKES_BOUNDS,
    }
    selected_bounds = lake_bounds.get(args.lake, LAKE_MICHIGAN_BOUNDS)
    print(f"Lake: {args.lake.capitalize()}")
    print(f"Bounds: lat {selected_bounds['south']}-{selected_bounds['north']}, lon {selected_bounds['west']}-{selected_bounds['east']}")
    print()
    ...
    for window in date_windows:
        ...
        # Search USGS Landsat
        if usgs_logged_in:
            scenes = usgs.search_scenes(selected_bounds, window)
        ...
        if sentinel_logged_in:
            bbox = [
                selected_bounds["west"],
                selected_bounds["south"],
                selected_bounds["east"],
                selected_bounds["north"]
            ]
