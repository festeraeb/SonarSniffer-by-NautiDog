...
class WreckHunterCalibrator:
    ...
    def _snr_from_patches(self, path: str, patch_size: int = 1024) -> dict:
        """Compute pure native-resolution SNR for a single band via streaming patches.
        ...
        with rasterio.open(path) as src:
            ...
            # WCS anchor: record bottom-left corner from transform
            t = src.transform
            origin = {'col_off': t.c, 'row_off': t.f, 'res_x': t.a, 'res_y': t.e}
            for y in range(0, h, patch_size):
                for x in range(0, w, patch_size):
                    self.gpu_cooldown_check(f'snr_patch {x},{y}')
                    win = Window(x, y, min(patch_size, w - x), min(patch_size, h - y))
                    patch = torch.from_numpy(
                        np.nan_to_num(src.read(1, window=win).astype('float32'),
                                      nan=0.0, posinf=0.0, neginf=0.0)
                    ).to(self.device)
                    if len(sample_vals) < sample_limit:
                        sample_vals.extend(
                            patch.cpu().flatten().tolist()[:max(0, sample_limit - len(sample_vals))]
                        )
        samples = np.asarray(sample_vals, dtype=np.float32)
        p001, p999 = float(np.percentile(samples, 0.1)), float(np.percentile(samples, 99.9))
        denom = (p999 - p001) if (p999 - p001) > eps else eps

        # second pass: mean/std of stretched values
        n, s, sq = 0, 0.0, 0.0
        with rasterio.open(path) as src:
            h, w = src.height, src.width
            for y in range(0, h, patch_size):
                for x in range(0, w, patch_size):
                    ...
                    stretched = torch.clamp((patch - p001) / denom, 0.0, 1.0)
                    n  += stretched.numel()
                    s  += float(stretched.sum().item())
                    sq += float((stretched * stretched).sum().item())
        mean = s / n
        std  = math.sqrt(max((sq / n) - mean * mean, eps))
        return {'p001': p001, 'p999': p999, 'mean': mean, 'std': std,
                'snr': mean / (std if std > eps else eps), 'origin': origin,
                'shape': (h, w)}
    ...
    def dual_stream_native_snr(self, b04_path: str, b05_path: str,
        ...
        b04_stats = self._snr_from_patches(b04_path, patch_size)
        ...
        b05_stats = self._snr_from_patches(b05_path, patch_size)
        ...
        with rasterio.open(b04_path) as src4, rasterio.open(b05_path) as src5:
            h5, w5 = src5.height, src5.width   # 20m grid is the coarse reference
            for y5 in range(0, h5, patch_size):
                for x5 in range(0, w5, patch_size):
                    self.gpu_cooldown_check(f'turb {x5},{y5}')
                    pw5 = min(patch_size, w5 - x5)
                    ph5 = min(patch_size, h5 - y5)
                    win5 = Window(x5, y5, pw5, ph5)

                    # B05 at native 20m
                    b05_patch = torch.from_numpy(
                        np.nan_to_num(src5.read(1, window=win5).astype('float32'),
                                      nan=0.0, posinf=0.0, neginf=0.0)
                    ).to(self.device)

                    # B04 at corresponding 10m window (2x pixels each axis)
                    win4 = Window(x5 * 2, y5 * 2, pw5 * 2, ph5 * 2)
                    b04_patch_10m = torch.from_numpy(
                        np.nan_to_num(src4.read(1, window=win4).astype('float32'),
                                      nan=0.0, posinf=0.0, neginf=0.0)
                    ).to(self.device)

                    # Downsample B04 to 20m by 2x2 average — real data → coarse stat
                    # reshape (H*2, W*2) → (H, 2, W, 2) → mean over 2x2 blocks
                    h4, w4 = b04_patch_10m.shape
                    h4e, w4e = (h4 // 2) * 2, (w4 // 2) * 2
                    b04_coarse = (b04_patch_10m[:h4e, :w4e]
                                  .reshape(h4e // 2, 2, w4e // 2, 2)
                                  .mean(dim=(1, 3)))

                    # align shapes (edge patches may differ by 1 pixel)
                    ch, cw = min(b04_coarse.shape[0], b05_patch.shape[0]), \
                             min(b04_coarse.shape[1], b05_patch.shape[1])
                    b04_c = b04_coarse[:ch, :cw]
                    b05_p = b05_patch[:ch, :cw]

                    turb_patch = b04_c / (b05_p + eps)
                    turb_sum += float(turb_patch.sum().item())
                    turb_n   += turb_patch.numel()

                    # Rossa signature: hard spike in B04_10m inside B05 glow
                    b04_norm = torch.clamp((b04_patch_10m - b04_p001) / b04_denom, 0.0, 1.0)
                    ...
                    # upsample B05 glow mask to 10m for overlay (result map, not raw data)
                    b05_glow_mask_20m = (b05_norm > b05_glow_thresh)
                    b05_glow_mask_10m = b05_glow_mask_20m.repeat_interleave(2, dim=0) \
    ...
    def calculate_turbidity_and_snr(self, b01_path, b04_path, mask_path=None, patch_size: int = 1024):
        ...
        # Try decreasing patch sizes on MemoryError
        for size in [patch_size, 512, 256, 128]:
            try:
                return self._calculate_stream(b01_path, b04_path, mask_path, size)
            except MemoryError:
                warnings.warn(f"MemoryError with patch {size}; trying smaller patch")
                continue
        raise MemoryError("Unable to process rasters within memory limits; try smaller patches or increase RAM")

    def _calculate_stream(self, b01_path, b04_path, mask_path, patch_size):
        eps = 1e-6
        sample_vals = []
        sample_limit = 200000
        turbidity_sum = 0.0
        ...
        # First pass: collect samples for quantiles and turbidity sum using GPU operations
        with rasterio.open(b01_path) as src1, rasterio.open(b04_path) as src4:
            h = src1.height
            w = src1.width
            if src4.height != h or src4.width != w:
                raise RuntimeError("B01/B04 dimension mismatch")

            for y in range(0, h, patch_size):
                for x in range(0, w, patch_size):
                    self.gpu_cooldown_check(f"patch {x},{y}")
                    win = Window(x, y, min(patch_size, w - x), min(patch_size, h - y))
                    b01_patch_np = src1.read(1, window=win).astype('float32')
                    b04_patch_np = src4.read(1, window=win).astype('float32')

                    b01_patch = torch.from_numpy(np.nan_to_num(b01_patch_np, nan=0.0, posinf=0.0, neginf=0.0)).to(self.device).float()
                    b04_patch = torch.from_numpy(np.nan_to_num(b04_patch_np, nan=0.0, posinf=0.0, neginf=0.0)).to(self.device).float()

                    turb_map = b04_patch / (b01_patch + eps)
                    turbidity_sum += float(turb_map.sum().item())
                    turbidity_count += int(turb_map.numel())

                    # reservoir-like sampling (collect up to sample_limit)
                    if len(sample_vals) < sample_limit:
                        sample_vals.extend(b01_patch.cpu().flatten().tolist()[: max(0, sample_limit - len(sample_vals))])
                    # allow Python to raise MemoryError if arrays too large

        if turbidity_count == 0:
            raise RuntimeError("No pixels processed")
        mean_turbidity = turbidity_sum / turbidity_count

        # compute p01/p99 from samples
        samples = np.asarray(sample_vals, dtype=np.float32)
        if samples.size == 0:
            raise RuntimeError("No sample values for quantile estimation")
        p01, p99 = np.percentile(samples, [0.1, 99.9])
        denom = (p99 - p01) if (p99 - p01) > eps else eps

        # Second pass: compute mean and std for stretched b01 on GPU
        total_pixels = 0
        sum_unmasked = 0.0
        sum_unmasked_sq = 0.0
        with rasterio.open(b01_path) as src1:
            ...
            for y in range(0, h, patch_size):
                for x in range(0, w, patch_size):
                    self.gpu_cooldown_check(f"snr patch {x},{y}")
                    win = Window(x, y, min(patch_size, w - x), min(patch_size, h - y))
                    b01_patch_np = src1.read(1, window=win).astype('float32')

                    b01_patch = torch.from_numpy(np.nan_to_num(b01_patch_np, nan=0.0, posinf=0.0, neginf=0.0)).to(self.device).float()
                    unmasked = torch.clamp((b01_patch - p01) / denom, 0.0, 1.0)

                    unmasked_sum = float(unmasked.sum().item())
                    unmasked_sq_sum = float((unmasked * unmasked).sum().item())

                    total_pixels += int(unmasked.numel())
                    sum_unmasked += unmasked_sum
                    sum_unmasked_sq += unmasked_sq_sum

        if total_pixels < 1:
            raise RuntimeError("No pixels for SNR computation")

        mean = sum_unmasked / total_pixels
        variance = (sum_unmasked_sq / total_pixels) - (mean * mean)
        variance = max(variance, eps)
        std = math.sqrt(variance)
        snr = mean / (std if std > eps else eps)

        return float(mean_turbidity), float(snr)
