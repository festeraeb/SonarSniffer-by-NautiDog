...
# The exact scene that worked yesterday — 2025-09-16, 0.001% cloud, Lake Michigan
TARGET_SCENE_ID   = 'S2C_16TDN_20250916_0_L2A'
TARGET_SCENE_DATE = '2025-09-16'
MGRS_TILE         = '16TDN'
...
def _cached(scene_id: str, band: str) -> Path | None:
    p = CACHE_DIR / _s2_filename(scene_id, band)
...
def _download_via_nasa(band: str, year_window: str, dest: Path) -> Path:
    """
    ...
    raise RuntimeError(f'NASA fallback exhausted for band={band} window={year_window}')


def download_band(tile: str, band: str, year_window: str = 'rossa') -> Path:
    """
    ...
    try:
        ...
        if not item:
            print(f'[!] Scene not found by ID, searching by date {TARGET_SCENE_DATE}...')
            item = _stac_search_by_date(MGRS_TILE, TARGET_SCENE_DATE)
        if item:
    ...
    return _download_via_nasa(band, year_window, dest)

# ── Public API ────────────────────────────────────────────────────────────────

def ensure_tiles(year_window: str = 'rossa') -> dict:
    """
    Download all bands for the target scene and return {lake: {band: Path}}.
    All bands come from the same scene (spatial consistency for surface target).
    """
    scene_id = TARGET_SCENE_ID
    lake = 'Michigan_16TDN'
    ...
    for band in BANDS:
        if _cached(scene_id, band) is None:
            download_band(MGRS_TILE, band, year_window)
