...
function Fail($Message) { Write-Host "`nFATAL ERROR: $Message" -ForegroundColor Red; Write-Host "Please check the logs above for details on this failure." -ForegroundColor Yellow; Read-Host "Press [Enter] to exit..."; exit 1 } # "ERROR: $Message" -ForegroundColor Red; exit 1 }
...
Write-Step "Building sonarsniffer_overlay sidecar"
cargo build --release --bin sonarsniffer_overlay
if ($LASTEXITCODE -ne 0) {
    Fail "sonarsniffer_overlay release build failed."
}

$sourceExe = Join-Path $root "target\release\sonarsniffer_overlay.exe"
if (-not (Test-Path $sourceExe)) {
    Fail "Expected sonarsniffer_overlay binary was not produced: $sourceExe"
}
...
$sidecarExe = Join-Path $sidecarDir ("sonarsniffer_overlay-{0}.exe" -f $targetTriple)
