...
pub const GRID_CELL_SIZE_M: u32 = 2000;
...
pub fn generate_grid(area: &SearchArea, cell_size: u32) -> Vec<GridCell> {
    // Convert all four corners to get proper bounds
    let (e_min, n_min, _) = wgs84_to_utm(area.lat_min, area.lon_min);
    let (e_max, n_max, _) = wgs84_to_utm(area.lat_max, area.lon_max);
    let (e_test1, n_test1, _) = wgs84_to_utm(area.lat_min, area.lon_max);
    let (e_test2, n_test2, _) = wgs84_to_utm(area.lat_max, area.lon_min);
    
    // Get actual min/max from all corners
    let easting_min = e_min.min(e_max).min(e_test1).min(e_test2);
    let easting_max = e_min.max(e_max).max(e_test1).max(e_test2);
    let northing_min = n_min.min(n_max).min(n_test1).min(n_test2);
    let northing_max = n_min.max(n_max).max(n_test1).max(n_test2);
