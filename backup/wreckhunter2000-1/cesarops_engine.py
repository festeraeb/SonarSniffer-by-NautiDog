...
# Portable drive OR local data folder (cross-platform)
# Priority: env var > MAC-based network discovery > local repo folder
_DB_NAME = "LAKE_MICHIGAN_CENSUS_2026.db"

try:
    from drive_discovery import find_armor_drive as _find_armor_drive
    _discovered = _find_armor_drive(quiet=True)
    DRIVE_PATH = _discovered if _discovered else Path(os.environ.get("CESAROPS_DRIVE", Path(__file__).parent))
except Exception:
    DRIVE_PATH = Path(os.environ.get("CESAROPS_DRIVE", Path(__file__).parent))

DB_PATH = DRIVE_PATH / _DB_NAME

# TPU Server
# Laptop: points to i7 (10.0.0.56) — Xeon offline, TPU+P1000 moved to i7
...
def run_test_pipeline():
    ...
    search_paths = [
        ...
        DRIVE_PATH / "outputs",
        DRIVE_PATH / "data",
        Path(os.environ.get("CESAROPS_DATA_DIR", DRIVE_PATH / "downloads")),
    ]
