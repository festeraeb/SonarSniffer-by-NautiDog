...
pub async fn trigger_swarm_download(
    ...
    let script = std::path::PathBuf::from(&cwd)
        .join("pipelines").join("satellite").join("batch_download_manager.py");
    let script_str = if script.exists() {
        script.to_string_lossy().to_string()
    } else {
        // fallback: bare name in case CWD already contains it
        "batch_download_manager.py".to_string()
    };
