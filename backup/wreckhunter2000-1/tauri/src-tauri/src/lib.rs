...
fn provider_runtime_config(
    ...
    match provider {
        ...
        "koboldcpp" => {
            let base = if let Some(b) = env_or_dotenv(&dotenv, "KOBOLDCPP_BASE_URL") {
                b
            } else {
                let local = "http://127.0.0.1:5001/v1".to_string();
                if tcp_reachable(&local) {
                    local
                } else {
                    // Cloudflare tunnel fallback: T440 via cesarops.org
                    let ts_ip = env_or_dotenv(&dotenv, "KOBOLDCPP_TAILSCALE_IP")
                        .or_else(|| env_or_dotenv(&dotenv, "T440_TAILSCALE"))
                        .unwrap_or_else(|| "100.72.182.77".to_string());
                    let tunnel_url = "https://llm.cesarops.org/v1".to_string();
                    let ts_url = format!("http://{}:5001/v1", ts_ip);
                    // Prefer tunnel, fall back to Tailscale direct, then local
                    if tcp_reachable(&tunnel_url) { tunnel_url }
                    else if tcp_reachable(&ts_url) { ts_url }
                    else { local }
                }
            };
            let model = normalized_override.unwrap_or_else(|| {
        ...
        "github_sdk" => {
            let base = env_or_dotenv(&dotenv, "GITHUB_MODELS_BASE_URL")
                .unwrap_or_else(|| "https://models.inference.ai.azure.com".to_string());
            let model = normalized_override.unwrap_or_else(|| {
                env_or_dotenv(&dotenv, "GITHUB_MODEL").unwrap_or_else(|| "gpt-4.1".to_string())
            });
            let has_key = env_or_dotenv(&dotenv, "GITHUB_TOKEN").is_some()
                || env_or_dotenv(&dotenv, "GITHUB_PAT").is_some();
            Ok((base, model, has_key))
        }
        "gemini" => {
...
fn agent_provider_env(
    ...
    match provider {
        ...
        "github_sdk" => {
            // GitHub Models is OpenAI-compatible — route through QWEN call path in ai_director.py
            env.insert("QWEN_BASE_URL".to_string(), base);
            env.insert("QWEN_MODEL".to_string(), model);
            let token = env_or_dotenv(&dotenv, "GITHUB_TOKEN")
                .or_else(|| env_or_dotenv(&dotenv, "GITHUB_PAT"))
                .unwrap_or_else(|| String::new());
            env.insert("QWEN_API_KEY".to_string(), token);
        }
        "gemini" => {
...
#[tauri::command]
async fn open_output_path(path: String) -> Result<(), String> {
