...
fn probe_nvml(node_id: &str) -> Option<HardwareCaps> {
    ...
    let fp64_tags = ["P100", "P40", "V100", "A100", "A40", "Quadro", "Tesla"];

    for i in 0..device_count {
        if let Ok(dev) = nvml.device_by_index(i) {
            if let Ok(mem) = dev.memory_info() {
                let gpu_gb = (mem.total / (1024 * 1024 * 1024)) as u32;
                total_vram_gb += gpu_gb;

                if let Ok(name) = dev.name() {
                    gpu_names.push(format!("GPU{}: {} ({}GB)", i, name, gpu_gb));
                    if fp64_tags.iter().any(|tag| name.contains(tag)) {
                        has_fp64 = true;
                    }
                }
            }
    ...
    let gpu_list = gpu_names.join(" + ");
    info!("NVML: {} | {}GB total | FP64: {}", gpu_list, total_vram_gb, has_fp64);
    Some(HardwareCaps {
...
async fn daemon_health() -> impl IntoResponse {
    (StatusCode::OK, Json(serde_json::json!({ "status": "ok", "service": "model-team-tool" })))
}
