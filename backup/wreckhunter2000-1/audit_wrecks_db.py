...
def resolve_wrecks_db() -> Path:
    here = Path(__file__).resolve().parent
    candidates = [
        here / "db" / "wrecks.db",
        here.parent / "db" / "wrecks.db",
        here.parent / "Bagrecovery" / "db" / "wrecks.db",
        Path(r"C:\Users\thomf\programming\Bagrecovery\db\wrecks.db"),
    ]
    for p in candidates:
        if p.exists():
            return p
...
def run_audit():
    ...
    for name, lat, lon in singletons[:50]:
        safe_name = str(name) if name is not None else "(unnamed)"
        safe_lat = f"{lat:.4f}" if lat is not None else "None"
        safe_lon = f"{lon:.4f}" if lon is not None else "None"
        print(f"  {safe_name:<45} lat={safe_lat}  lon={safe_lon}")
