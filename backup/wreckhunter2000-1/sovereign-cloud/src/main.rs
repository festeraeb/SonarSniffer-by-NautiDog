...
const SELF_TRAIN_DB_PATH: &str = "./data/kobold.db";
...
async fn main() -> Result<()> {
    ...
    // 4c. Build self-training SQLite store for scan results and research metadata
    let self_training_db = Arc::new(self_training::SelfTrainingDb::open(SELF_TRAIN_DB_PATH)?);
    info!("SelfTrainingDb: opened SQLite store at {}", SELF_TRAIN_DB_PATH);
