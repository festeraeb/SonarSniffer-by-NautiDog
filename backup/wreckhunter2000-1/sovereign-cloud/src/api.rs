...
use crate::self_training::{ScanRequest, ScanResult, SelfTrainingDb};
...
pub struct NodeState {
    ...
    pub discovery: Option<Arc<crate::discovery::NodeDiscovery>>,    /// Self-training database for scan results and research
    pub scan_store: Arc<SelfTrainingDb>,    /// Laptop mode — node is paused, idle scanner stopped, minimal CPU/GPU use
...
pub fn router(state: Arc<NodeState>) -> Router {
    Router::new()
        ...
        .route("/v1/node/hardware", get(node_hardware))
...
async fn chat_completions(
    ...
    let reply = if let Some(tt) = task_type {
        ...
        // LLM mode — route to appropriate backend based on model
        state.swap_mode(ComputeMode::Llm).await;
        
        // Default backend
        let mut llm_base = std::env::var("LLM_BASE_URL")
            .or_else(|_| std::env::var("KOBOLD_BASE_URL"))
            .or_else(|_| std::env::var("OLLAMA_BASE_URL"))
            .unwrap_or_else(|_| "http://localhost:5001/v1".into());
        
        // Model-specific backends from LLM_BACKENDS env var (JSON map)
        if let Ok(backends_str) = std::env::var("LLM_BACKENDS") {
            if let Ok(backends) = serde_json::from_str::<std::collections::HashMap<String, String>>(&backends_str) {
                if let Some(backend) = backends.get(&model) {
                    llm_base = backend.clone();
                }
            }
        }
...
async fn cluster_llm_recommend(State(state): State<Arc<NodeState>>) -> impl IntoResponse {
    ...
    let catalogue = vec![
        // ── Phi-3 series (small, fits on P1000 4GB) ──────────────────────────
        ("Phi-3-Mini-3.8B Q4",    "microsoft/Phi-3-mini-4k-instruct-gguf", "Phi-3-mini-4k-instruct-q4_0.gguf", 3u32, 4u32, "Q4_0"),
        ("Phi-3-Mini-3.8B FP16",  "microsoft/Phi-3-mini-4k-instruct-gguf", "Phi-3-mini-4k-instruct-fp16.gguf", 8, 4, "FP16"),
        ...
        // ── Qwen2.5 35B AWQ — fits in 32GB with KV cache at Q8 ────────────────
        // Uses AWQ (Activation-aware Weight Quantization) — better quality than
        // standard Q4 at the same size. KV cache at Q8 keeps reasoning quality.
        ("Qwen2.5-Coder-32B AWQ",  "Qwen/Qwen2.5-Coder-32B-Instruct-AWQ",   "qwen2.5-coder-32b-instruct-awq.gguf",     22,    32,    "AWQ"),
        // ── DeepSeek-R1 distill — reasoning in one model ─────────────────────
        ...
        ("DeepSeek-R1-Distill-7B Q4", "bartowski/DeepSeek-R1-Distill-Qwen-7B-GGUF", "DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf", 6, 7, "Q4_K_M"),
        // ── TurboQuant fork — higher throughput on P100 FP16 path ────────────
        // bartowski's imatrix quants use calibration data for better quality
        ("Qwen2.5-Coder-32B IQ4",  "bartowski/Qwen2.5-Coder-32B-Instruct-GGUF", "Qwen2.5-Coder-32B-Instruct-IQ4_XS.gguf", 18, 32, "IQ4_XS"),
        ("Qwen2.5-Coder-14B IQ4",  "bartowski/Qwen2.5-Coder-14B-Instruct-GGUF", "Qwen2.5-Coder-14B-Instruct-IQ4_XS.gguf",  9, 14, "IQ4_XS"),
    ];
...
/// Hardware telemetry — GPU stats via NVML, CPU temps via sysfs.
async fn node_hardware(State(state): State<Arc<NodeState>>) -> impl IntoResponse {
    // GPU stats via NVML
    let gpus = collect_gpu_stats();

    // CPU temp via sysfs (Linux)
    let cpu_temp_c = read_cpu_temp();

    // CPU utilization via /proc/stat
    let cpu_util_pct = read_cpu_util();

    // RAM
    let (ram_total_gb, ram_used_gb) = read_ram();

    (StatusCode::OK, Json(serde_json::json!({
        "node_id": state.allocation.capabilities.read().await.node_id,
        "gpus": gpus,
...
fn collect_gpu_stats() -> Vec<serde_json::Value> {
    ...
    for i in 0..count {
        if let Ok(dev) = nvml.device_by_index(i) {
            ...
            let power_w = dev.power_usage().ok().map(|p| p / 1000);
            let power_limit_w = dev.enforced_power_limit().ok().map(|p| p / 1000);
            gpus.push(serde_json::json!({
                "index": i,
                "name": name,
                "temp_c": temp,
                "util_pct": util.as_ref().map(|u| u.gpu),
...
fn read_cpu_temp() -> Option<f32> {
    // Try hwmon sysfs — works on most Linux systems
    for i in 0..10 {
        let path = format!("/sys/class/hwmon/hwmon{}/temp1_input", i);
        if let Ok(s) = std::fs::read_to_string(&path) {
            if let Ok(millic) = s.trim().parse::<f32>() {
                return Some(millic / 1000.0);
            }
        }
    }
...
fn read_cpu_util() -> Option<f32> {
    // Read /proc/stat twice 200ms apart for a real utilization sample
...
fn read_ram() -> (f32, f32) {
    if let Ok(s) = std::fs::read_to_string("/proc/meminfo") {
        ...
        let used_gb = (total - available) as f32 / 1024.0 / 1024.0;
...
/// Self-training scan endpoint — accepts request, spawns scan tasks, stores results in database.
async fn handle_scan(
    State(state): State<Arc<NodeState>>,
    Json(req): Json<ScanRequest>,
) -> impl IntoResponse {
    info!("Scan request: gpus={}, research_mode={}", req.gpus.len(), req.research_mode);

    let mut results = Vec::new();
    
    for (idx, gpu_name) in req.gpus.iter().enumerate() {
        // Simulate scan processing per GPU
        let confidence = (0.5 + (idx as f32 * 0.1)).clamp(0.0, 1.0);
        let scan_type = if req.research_mode { "research" } else { "standard" };
        
        let result = ScanResult {
            id: None,
            gpu: gpu_name.clone(),
            scan_type: scan_type.to_string(),
            result: format!("Scan completed with confidence {:.2}", confidence),
            research_mode: req.research_mode,
            research_notes: if req.research_mode { 
                "Research mode enabled — cross-referencing with scientific literature".to_string() 
            } else { 
                String::new() 
            },
        ...
        if let Err(e) = state.scan_store.insert_scan_result(&result) {
            warn!("Failed to store scan result: {}", e);
        }
        
        results.push(result);
    }
    
    info!("Scan completed: {} results stored", results.len());
