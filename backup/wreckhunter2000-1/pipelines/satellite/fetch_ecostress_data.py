...
ECOSTRESS_COLLECTION_ID = "C1234567891-POCLOUD"  # TODO: Replace with actual ECOSTRESS concept ID


def fetch_ecostress_data(bbox: Tuple[float, float, float, float], time_range: Tuple[str, str], output_dir: str) -> str:
    """Fetch ECOSTRESS data for the given bounding box and time range."""
    client = EarthdataClient()

    granules = client.search_granules(
        collection_concept_id=ECOSTRESS_COLLECTION_ID,
        bbox=bbox,
    ...
    output_path = os.path.join(output_dir, "ecostress_data.nc")
...
if __name__ == "__main__":
    ...
    output_dir = "../sample_data/ecostress"

    os.makedirs(output_dir, exist_ok=True)

    # EarthdataClient handles token auth internally.
    try:
        file_path = fetch_ecostress_data(bbox, time_range, output_dir)
        print(f"Data saved to {file_path}")
    except Exception as e:
        print(f"Error fetching ECOSTRESS data: {e}")