...
def _curvelet_denoise(
    ...
    if len(bounds) >= 2:
        t = (r - bounds[0]) / (bounds[1] - bounds[0] + 1e-15)
        w_coarse = np.sqrt(np.clip(1.0 - _meyer_nu(t), 0, 1))
    else:
    ...
    for j in range(1, len(bounds) - 1):
        ...
        rising = np.sqrt(np.clip(_meyer_nu((r - lo) / (mid - lo + 1e-15)), 0, 1))
        falling = np.sqrt(np.clip(1.0 - _meyer_nu((r - mid) / (hi - mid + 1e-15)), 0, 1))
        windows.append(rising * falling)

    # Fine window: everything above bounds[-2]
    if len(bounds) >= 2:
        t = (r - bounds[-2]) / (bounds[-1] - bounds[-2] + 1e-15)
        w_fine = np.sqrt(np.clip(_meyer_nu(t), 0, 1))
    else:
