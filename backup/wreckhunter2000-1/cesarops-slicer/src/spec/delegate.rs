...
pub enum DelegateTarget {
    ...
    /// Vulkan GPU (Compute shaders, cross-platform)
    #[serde(rename = "VULKAN_GPU")]
    VulkanGpu,
...
impl DelegateTarget {
    ...
    pub fn from_str(s: &str) -> Self {
        match s.to_uppercase().as_str() {
            ...
            "VULKAN_GPU" | "GPU" | "VULKAN" => Self::VulkanGpu,
            "HYBRID" => Self::Hybrid,
            "SKIP" => Self::Skip,
            _ => Self::Cpu,
        }
    ...
    pub fn needs_gpu(&self) -> bool {
        matches!(self, Self::VulkanGpu | Self::Hybrid)
    }
...
impl DelegateRouter {
    ...
    pub fn route(&mut self, tile_id: &str, delegate: DelegateTarget) {
        match delegate {
            ...
            DelegateTarget::VulkanGpu => self.gpu_tiles.push(tile_id.to_string()),
    ...
    pub fn total(&self) -> usize {
        self.cpu_tiles.len() + self.tpu_tiles.len() + self.gpu_tiles.len() + self.skipped.len()
    }
