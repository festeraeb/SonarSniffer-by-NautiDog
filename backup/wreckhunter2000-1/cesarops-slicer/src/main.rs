...
use cesarops_slicer::io::vrt::{ResampleMethod, VrtDataset, VrtNormalizer, VrtSource};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Slice {
            ...
            info!(
                "Parsed: {}x{}, {} bands",
                tiff.width, tiff.height, tiff.band_count
            );
            ...
            info!("Slicing with tile_size={} → {:?}", tile_size, output);
        ...
        Commands::Run {
            ...
            for input in &inputs {
                ...
                let input = if gdal_warp::is_projected_crs(input) {
                    info!("  Detected projected CRS (SAR/UTM) — warping to EPSG:4326 first...");
                ...
                let pixel_deg = gdal_warp::gdalinfo_geo_transform(&input)
                    ...
                    .unwrap_or_else(|| {
                        ...
                        let span =
                            (spec.search_params.bounds[0] - spec.search_params.bounds[2]).abs();
                        // Rough: 10m at center lat scaled to degrees
                        let center_lat =
                            (spec.search_params.bounds[0] + spec.search_params.bounds[2]) / 2.0;
                ...
                for entry in &manifest.tiles {
                    let delegate =
                        cesarops_slicer::spec::delegate::DelegateTarget::from_str(&entry.delegate);
