...
pub struct VrtTileManifest {
    ...
    /// Tiles that failed to slice. Logged at WARN and collected here instead
    /// of silently dropping with filter_map(.ok()).
    pub failed_tiles: Vec<FailedVrtTileEntry>,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct FailedVrtTileEntry {
    pub grid_col: usize,
    pub grid_row: usize,
    pub error: String,
}
...
impl VrtTileSlicer {
    ...
    pub fn from_vrt_file<P: AsRef<std::path::Path>>(
        ...
        let vrt = VrtDataset::from_file(&vrt_path).context("failed to parse VRT file")?;
    ...
    pub fn slice_all(&self, mission: Option<&MissionSpec>) -> Result<VrtTileManifest> {
        ...
        // Parallel tile generation — collect Results so failures are visible
        let raw: Vec<(usize, usize, Result<VrtSlicedTile>)> = (0..tile_rows)
            .into_par_iter()
            .flat_map_iter(move |ty| {
                (0..tile_cols)
                    .map(move |tx| (tx, ty, self.slice_tile(tx, ty, &anchor_calc, mission)))
            })
            .collect();

        let mut tiles = Vec::with_capacity(raw.len());
        let mut failed_tiles: Vec<FailedVrtTileEntry> = Vec::new();
        for (tx, ty, result) in raw {
            match result {
                Ok(tile) => tiles.push(tile),
                Err(e) => {
                    warn!("VRT tile ({tx},{ty}) failed to slice: {e}");
                    failed_tiles.push(FailedVrtTileEntry {
                        grid_col: tx,
                        grid_row: ty,
                        error: e.to_string(),
                    });
        ...
        if !failed_tiles.is_empty() {
            warn!(
                "{} VRT tile(s) failed and are recorded in manifest.failed_tiles",
                failed_tiles.len()
            );
        }
        ...
        info!(
            "VRT sliced {} tiles → {:?}",
            manifest.tile_count, self.output_dir
        );
        Ok(manifest)
    }
    ...
    fn slice_tile(
        ...
        debug!("VRT Tile ({tx},{ty}): id={tile_id} delegate={:?}", delegate);
...
impl VrtTileSlicer {
    ...
    pub fn measure_drift(
        ...
        manifest
            ...
            .map(|entry| {
                ...
                let expected_lon = gt[0] + (entry.grid_col as f64 * tile_size * gt[1]);
                let expected_lat = gt[3] + (entry.grid_row as f64 * tile_size * gt[5]);
                ...
                let drift_m = haversine_meters(expected_lat, expected_lon, actual_lat, actual_lon);
    ...
    pub fn detect_overlaps(&self, manifest: &VrtTileManifest) -> Vec<TileOverlap> {
        ...
        for entry in &manifest.tiles {
            ...
            if let Some(east) = tile_map.get(&(col + 1, row)) {
                ...
                let overlap_lat =
                    this_south_lat.min(east_south_lat) - entry.origin_lat.max(east.origin_lat);
            ...
            if let Some(south) = tile_map.get(&(col, row + 1)) {
                ...
                let overlap_lon =
                    this_east_lon.min(south_east_lon) - entry.origin_lon.max(south.origin_lon);
