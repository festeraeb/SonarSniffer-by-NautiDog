...
AGENT_CONFIG = {
    "areas": {
        ...
        "lake_erie_full": {
            ...
            "label": "Lake Erie Full (HC Leak Oct 2015)"
        },
        "lake_erie_central_basin": {
            "bbox": [41.80, -82.50, 42.50, -80.00],
            "label": "Lake Erie Central Basin (Marquette & Bessemer No. 2)"
        }
    },
    "sensors": ["thermal", "nir_swir", "sar", "swot", "bathymetry"],
    "thresholds": {
        "thermal_zscore": 2.5,
        "sar_coherence": 0.6,
        "glint_ratio_b08_b04": 1.5,
        "swot_ssh_m": 0.015,
        "bathymetry_anomaly": 0.8
    },
    "execution": {
        "cpu_fallback": False,
        "timeout_min": 90,
...
def llm_generate_sensor_plan(user_request: str) -> dict:
    ...
    system_msg = (
        ...
        f"Available sensors: thermal, nir_swir, sar, swot, bathymetry\n\n"
        f"Respond with ONLY valid JSON matching this schema:\n"
        '{{"area": "<area_key>", "sensors": ["sensor", ...], "thresholds": {{"thermal_zscore": N, "bathymetry_anomaly": Mthymetry_anomaly": M}}, "reasoning": "..."}}'
    )
    raw = call_qwen([
...
def llm_interpret_results(results: list, config: dict) -> str:
    ...
    return call_qwen([
        {"role": "system", "content": (
            ...
            "Assume a Zero-Disk architecture where imagery is streamed directly into memory.\n"
...
def discover_stac_scenes(bbox, collection="sentinel-1-grd", start_date="2024-01-01", end_date="2024-01-31"):
    """
    Query Microsoft Planetary Computer for STAC scenes via zero-disk API streaming.
    Returns a list of URLs pointing to the cloud-optimized geotiffs.
    """
    try:
        ...
        return [item.assets.get("vh").href for item in search.items() if "vh" in item.assets]
    except Exception as e:
        log(f"⚠️ STAC Query failed: {e}", "WARN")
        return []

def run_sensor_tool(sensor_name, area_cfg, thresholds):
    """Execute a sensor tool. Returns dict with status & output path."""
    
    # Path to our new Zero-Disk Rust muscle binaries
    rust_release_dir = Path(__file__).parent / "cesarops-slicer" / "target" / "release"
    ...
    sensor_map = {
        ...
        "sar": (str(rust_release_dir / "sar_slick_worker.exe"), [
            "--coherence-min", str(thresholds.get("sar_coherence", 0.6)),
            # Bbox format used in the rust binary. We will pass a STAC URL if found
        ]),
        ...
        "bathymetry": (str(rust_release_dir / "bathymetry_specialist.exe"), [
            "--bag-url", "s3://dummy-bag-stream-url", 
            "--anomaly-threshold", str(thresholds.get("bathymetry_anomaly", 0.8)),
            "--scene-id", "BAG-SCENE-001"
        ])
    }
    ...
    script_path = Path(__file__).parent / script if not Path(script).is_absolute() else Path(script)
    ...
    if script_path.suffix.lower() == '.exe':
        ...
        # Ensure Zero-Disk architecture is enforced using STAC urls
        if sensor_name == "sar":
            # Attempt to fetch zero-disk STAC urls over HTTP for this region
            log(f"📡 Querying STAC for Zero-Disk direct memory streaming (SAR)...")
            stac_urls = discover_stac_scenes(area_cfg['bbox'])
            if stac_urls:
                # Add the most recent STAC url to the Rust arguments
                cmd.extend(["--stac-url", stac_urls[0]])
                log(f"🔗 Fed stac-client URL directly into Rust Memory Map: {stac_urls[0]}")
    else:
...
def main():
    ...
    if args.status or args.monitor:
        ...
        # ── Zero-Disk Architecture Note ─────────────────────────────
        if args.status:
            print("\n--- Data Inventory ---")
            print("✅ Zero-Disk architecture enabled. Data is streamed via STAC API memory-mapping.")
