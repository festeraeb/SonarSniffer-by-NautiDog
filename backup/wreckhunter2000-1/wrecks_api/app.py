...
# Set API_BASE_URL to your Cloudflare tunnel public URL so Google Earth
# can reach the live feed from anywhere.  Falls back to localhost.
_API_BASE_URL = os.environ.get("API_BASE_URL", "https://api.cesarops.org").rstrip("/")
...
def _get_scan_audit_db(write: bool = False):
    ...
    conn.row_factory = sqlite3.Row
    # Always ensure table exists BEFORE setting query_only
    conn.executescript('''
...
def mag_data_status():
    ...
    try:
        ...
        return {"status": "unavailable", "reason": "mag_data_manager module not installed on this node"}
...
def erie_wellheads(wells_csv: str = None, lake_erie_only: bool = True):
    ...
    try:
        ...
        return {"wells": [], "total": 0, "error": f"Module not available: {e}"}
...
def erie_known_wrecks():
    ...
    try:
        ...
        return {"wrecks": [], "total": 0, "error": f"Module not available: {e}"}
...
def azure_vision_status():
    ...
    try:
        ...
        return {"available": False, "reason": "bag_processor module not installed on this node"}
...
def datum_correct_single(req: DatumCorrectionRequest):
    """Apply NAD27 to WGS84 Molodensky + Loran-C rubber-sheet correction to a single point."""
    try:
        ...
        raise HTTPException(status_code=503, detail=f'datum_correction module not available: {e}')
...
def datum_list_anchors():
    ...
    try:
        ...
        return {'total': 0, 'ready': 0, 'anchors': [], 'error': f'Module not available: {e}'}
