...
impl ZeroAllocBpeTokenizer {
    ...
    pub fn encode(&self, text: &str) -> Vec<u32> {
        ...
        // Qwen tokenizer uses byte-level BPE with UTF-8 byte fallback
        let mut tokens: Vec<u32> = Vec::new();

        // Try to greedily match whole words/subwords first, then fall back to BPE
        let words = split_into_pretokens(text);
