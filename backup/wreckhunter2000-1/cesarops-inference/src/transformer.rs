...
impl TransformerDecoder {
    ...
    pub fn forward(
        ...
        for layer_idx in 0..self.config.num_layers {
            ...
            let mut q = matmul::matmul_f32_transposed_b(hidden_state, &q_weight, 1, h, q_dim);

            let k_name = format!("blk.{}.attn_k.weight", layer_idx);
            let k_weight = self.load_tensor_f32(weights, &k_name, kv_dim * h);
            let mut k = matmul::matmul_f32_transposed_b(hidden_state, &k_weight, 1, h, kv_dim);

            let v_name = format!("blk.{}.attn_v.weight", layer_idx);
            let v_weight = self.load_tensor_f32(weights, &v_name, kv_dim * h);
            let mut v = matmul::matmul_f32_transposed_b(hidden_state, &v_weight, 1, h, kv_dim);
            ...
            let o_out = matmul::matmul_f32_transposed_b(&attn_output, &o_weight, 1, q_dim, h);
            ...
            let gate = matmul::matmul_f32_transposed_b(hidden_state, &gate_weight, 1, h, inter);

            let up_weight = self.load_tensor_f32(weights, &up_name, inter * h);
            let up = matmul::matmul_f32_transposed_b(hidden_state, &up_weight, 1, h, inter);
            ...
            let down_out = matmul::matmul_f32_transposed_b(&ffn_hidden, &down_weight, 1, inter, h);
        ...
        matmul::matmul_f32_transposed_b(hidden_state, &lm_head, 1, h, self.config.vocab_size)
    }
