...
* **i7 (Ubuntu / 10.0.0.56):** The Workhorse. Runs GPU (Quadro P1000) + Coral TPU processing and scan analysis. Xeon is OFFLINE — its hardware has moved here.
  * **Work Dir:** `/home/cesarops/wreckhunter2000-1`
  * **GPU:** Quadro P1000 (4GB VRAM). Use CuPy for z-score/thresholding.
  * **TPU:** Coral TPU. tpu_server on port 5001.
  * **CPU:** 8-Core i7.
...
* **SSH:**
  ...
  * i7: `cesarops` / `cesarops` (10.0.0.56)
...
* TPU server runs on i7 at `http://10.0.0.56:5001`
...
## 📤 JOB QUEUE — REQUIRED OUTPUT FORMAT

When you submit a job to the Pi queue (`POST http://10.0.0.226:8099/jobs`), the body **must** include all required fields or the job will be picked up and immediately fail.

### Required fields

| Field | Type | Description |
|-------|------|-------------|
| `label` | string | Unique human name, e.g. `"hormuz_bottleneck"` |
| `bbox` | `[lat_min, lon_min, lat_max, lon_max]` | **4-element float list — NEVER empty** |
| `sensors` | list of strings | `["sar"]`, `["optical","sar"]`, etc. |
| `params.date_start` | `"YYYY-MM-DD"` | **Required** — start of data window |
| `params.date_end` | `"YYYY-MM-DD"` | **Required** — end of data window |
| `job_type` | `"cpu"` \| `"gpu"` \| `"gpu_tpu"` | Route to correct worker. SAR/optical → `"gpu"` |
| `priority` | int | 1 = normal, higher = runs first |

### Optional params fields

| Field | Default | Description |
|-------|---------|-------------|
| `params.target_type` | `""` | `"wreck"`, `"bathymetric"`, etc. |
| `params.mission_name` | label value | Human label for the mission |
| `pipeline_stage` | `"process"` | `"process"` or `"postprocess"` |

### Canonical example

```json
POST http://10.0.0.226:8099/jobs
{
  "label": "hormuz_bottleneck",
  "bbox": [26.1, 56.3, 26.8, 57.0],
  "sensors": ["sar"],
  "params": {
    "date_start": "2025-06-01",
    "date_end":   "2025-09-30",
    "target_type": "wreck",
    "mission_name": "Hormuz Bottleneck Survey"
  },
  "job_type": "gpu",
  "priority": 1
}
...
### Sensor → job_type rules

| Sensors in job | job_type |
|----------------|----------|
| `mag`, `bag`, `cpu-only` | `"cpu"` |
| `sar`, `optical`, `hls`, `thermal`, `nir_swir` | `"gpu"` |
| Triple-lock fusion + TPU inference | `"gpu_tpu"` |

### ⚠️ Failure modes if fields are missing

* `bbox: []` → `universal_downloader.py` is called with no bbox and fails immediately
* `params` missing `date_start`/`date_end` → job raises `ValueError` and marks FAILED in ~1 second
* `job_type` wrong (e.g. `"cpu"` for an optical job) → job will never be claimed by a GPU worker and sits in queue forever

### Checking queue status

```python
import urllib.request, json
data = json.loads(urllib.request.urlopen('http://10.0.0.226:8099/jobs', timeout=8).read())
for j in data.get('jobs', []):
    print(j['status'], j['label'], j['bbox'], j.get('params'))
```



## Deep Search Stack Details
- **SAR Denoising**: nauticuvs (Curvelets cleanup surface ripples to find the 'spine')
- **Plume Origins**: stac-client (Landsat-8/Sentinel-2 for tidal mapping)
- **Thermal Outliers**: scirs2-stats (Find statistically significant cold sinks)
- **Lidar Streaming**: copc-streaming (View bathymetry without massive downloads)



## Galvanic, Biological, and Physics Anomalies (Advanced Tier)
- **ICESat-2 (ATL03)**: hdf5/netcdf (Green-light photon laser bathymetry measuring stationary internal waves/water mounds over wrecks)
- **Sentinel-1 SAR**: stac-client (Detecting biogenic slicks and galvanic ion release dampening capillary waves/Bragg scattering)
- **PRISMA/DESIS Hyperspectral**: 240+ bands to detect chemical fingerprints (iron oxides/lead salts from galvanic cells)
- **VIIRS**: Thermal inertia (Comparing 2AM vs 2PM thermal lag for hot/cold sinks)
- **Sentinel-3 OLCI**: Persistent Chlorophyll-a / algae green spots indicating artificial reefs/zebra mussels
- **ERA5**: Subtracting wind/current noise for baseline calibration
- **GOCE/GRACE-FO**: Micro-gravity anomalies (Bouguer anomalies) for massive steel mass concentrations
