...
pub async fn fetch_sensor_metadata(
    short_name: &str,
) -> Result<SensorMeta, Box<dyn std::error::Error>> {
    ...
    let resolution = if let Some(item) = res["items"].get(0) {
        item["umm"]["SpatialExtent"]["HorizontalSpatialDomain"]["ResolutionAndCoordinateSystem"]
            ["HorizontalDataResolution"]["GenericResolutions"][0]["IncrementalHorizontalResolution"]
            .as_f64()
            .unwrap_or(10.0)
    } else {
