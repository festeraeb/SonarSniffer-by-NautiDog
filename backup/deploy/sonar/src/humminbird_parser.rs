...
fn read_i32_at(hdr: &[u8], offset: usize, big_endian: bool) -> Option<i32> {
    ...
    Some(if big_endian {
        be_i32(&hdr[offset..offset + 4])
    } else {
        le_i32(&hdr[offset..offset + 4])
    })
}
...
fn read_u32_at(hdr: &[u8], offset: usize, big_endian: bool) -> Option<u32> {
    ...
    Some(if big_endian {
        be_u32(&hdr[offset..offset + 4])
    } else {
        le_u32(&hdr[offset..offset + 4])
    })
}
...
fn parse_son_file(
    ...
    let Some(first_magic) = find_son_magic(&bytes, 0) else {
        return;
    };
    ...
    while pos + hlen <= bytes.len() {
        ...
        let heading = read_f32_at(hdr, 44).unwrap_or(0.0);
