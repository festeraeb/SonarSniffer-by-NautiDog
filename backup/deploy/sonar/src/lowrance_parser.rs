...
const LOWRANCE_SPHERE_R: f64 = 6_356_752.314_2;
...
pub fn parse_file(path: &Path) -> ParseResult {
    ...
    let frame_hdr_size = match format_id {
        ...
        _ => return err(&format!(
            "Unrecognised Lowrance format id {format_id:#06x} (expected 0x0002 SL2 or 0x0003 SL3)"
        )),
    };
    let format_name = if format_id == SL2_FORMAT {
        "SL2"
    } else {
        "SL3"
    };
