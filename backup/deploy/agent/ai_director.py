...
# GitHub Models (free via GitHub PAT — GITHUB_PAT or GITHUB_TOKEN in .env)
GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN",
    _dotenv.get("GITHUB_TOKEN",
    _dotenv.get("GITHUB_PAT", "")))
GITHUB_MODEL = os.environ.get("GITHUB_MODEL", _dotenv.get("GITHUB_MODEL", "gpt-4o-mini"))
GITHUB_MODELS_BASE_URL = os.environ.get("GITHUB_MODELS_BASE_URL", _dotenv.get("GITHUB_MODELS_BASE_URL",
...
# ── Register missions from missions.py into AVAILABLE_TOOLS ──────────────────
# This lets the AI Director discover and launch any run_mission.py mission
# by natural language request.

def _register_run_missions():
    """Load missions.py at startup and add each mission as a tool entry."""
    try:
        import missions as M
        for m_name, m in M.MISSIONS.items():
            tool_id = f"mission:{m_name}"
            AVAILABLE_TOOLS[tool_id] = {
                'script':          'run_mission.py',
                '_run_mission':    m_name,          # internal tag for dispatch
                'description':     m['label'],
                'default_threshold': None,
                'best_for':        [m['target_type']] + [
                    m_name.replace('_', ' '),
                    m['label'].split('—')[0].strip(),
                ],
            }
    except ImportError:
        pass  # missions.py not present — silently skip

_register_run_missions()
...
def _build_system_prompt() -> str:
    ...
    # ── Dynamically list run_mission.py missions from missions.py ─────────
    run_mission_tools = {tid: t for tid, t in AVAILABLE_TOOLS.items()
                         if tid.startswith("mission:") and '_run_mission' in t}
    if run_mission_tools:
        prompt += "\nGlobal / maritime scan missions (always use the exact tool ID):\n"
        for tid, t in run_mission_tools.items():
            prompt += f"  {tid} — {t['description']}\n"
        prompt += (
            "  Use these for Hormuz, mine hazard, Nome, Alaska, Nome Cessna, "
            "aircraft on ice, canopy scans, or any non-Great-Lakes target.\n"
        )
...
def call_github_models(messages: List[Dict]) -> str:
    """Send messages to GitHub Models via OpenAI-compatible API (uses GITHUB_PAT)."""
    if not GITHUB_TOKEN:
        raise RuntimeError(
            "GITHUB_TOKEN / GITHUB_PAT not set. Add GITHUB_PAT=ghp_... to .env"
        )
...
PROVIDER_REGISTRY = {
    ...
    "github_models": lambda: (GITHUB_TOKEN,       call_github_models),
}

# Fallback order when primary provider fails
PROVIDER_FALLBACK_ORDER = ["github_models", "qwen", "gemini", "groq", "anthropic"]
...
def call_llm(messages: List[Dict], preferred_provider: str = None) -> str:
    ...
    """
    # Build try-order: if explicitly requested, use only that provider.
    # Otherwise, use full fallback chain.
    if preferred_provider:
        if preferred_provider not in PROVIDER_REGISTRY:
            raise RuntimeError(f"Unknown provider: {preferred_provider}")
        order = [preferred_provider]
    else:
        order = list(PROVIDER_FALLBACK_ORDER)
...
def interpret_results_with_qwen(results: List[Dict], config: Dict, provider: str = None) -> str:
    ...
    messages = [
        ...
        {"role": "user", "content": (
            f"Search config:\n"
            f"  Area: {(config.get('bbox') or {}).get('name', 'Unknown')}\n"
...
class AIDirector:
    ...
    def parse_request(self, request: str) -> Dict:
        ...
        # ── Also check run_mission.py missions by keyword ─────────────────
        run_missions = {tid: t for tid, t in AVAILABLE_TOOLS.items()
                        if tid.startswith("mission:") and '_run_mission' in t}
        for tid, t in run_missions.items():
            search_terms = [t['_run_mission'].replace('_', ' '),
                            t['_run_mission'].replace('_', '')] + \
                           [b.lower() for b in t.get('best_for', [])]
            if any(term in request_lower for term in search_terms):
                tools = [tid]  # mission tool takes over — sole tool
                bbox_name = None  # bbox comes from missions.py config
                break
    ...
    def run_tool(self, tool_name: str) -> Dict:
        ...
        # ── run_mission.py dispatch (missions.py registry) ────────────────
        if '_run_mission' in tool:
            return self._run_external_mission(tool_name, tool)
    ...
    def _run_external_mission(self, tool_name: str, tool: dict) -> dict:
        """
        Dispatch to run_mission.py for missions defined in missions.py.
        The AI Director picks the mission; run_mission.py handles the rest.
        --queue pushes to i7. Default = local GPU/CPU run.
        """
        m_name = tool['_run_mission']
        runner = Path(__file__).parent / 'run_mission.py'
        if not runner.exists():
            return {'tool': tool_name, 'success': False,
                    'error': 'run_mission.py not found'}

        # Pass sensitivity as max_tiles proxy: aggressive = more tiles
        sens = self.config.get('sensitivity', 2.0)
        max_tiles = max(5, int(30 / sens))  # aggressive(1.0)→30, balanced(2.0)→15, strict(3.0)→10

        cmd = [sys.executable, str(runner), m_name,
        ...
        try:
            ...
            return {'tool': tool_name, 'success': False, 'error': 'Timed out (30 min)'}
        except Exception as e:
            return {'tool': tool_name, 'success': False, 'error': str(e)}
    ...
    def execute(self) -> List[Dict]:
        if not self.config.get('tools'):
            print("\n[AI Director] No tools selected — nothing to execute.")
            print("  Try: 'scan lake michigan for wrecks' or 'probe andaste with thermal'")
            return []
        if self.config.get('bbox') is None:
            print("\n[AI Director] No area specified — cannot execute scan.")
            print("  Try specifying a lake or bbox: 'scan lake superior' or '--bbox lake_michigan'")
            return []
        print(f"\n{'='*70}")
