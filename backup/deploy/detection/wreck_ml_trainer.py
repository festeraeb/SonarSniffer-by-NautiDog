...
SAR_LAND_CONFIG    = ROOT / "sar_land_config.json"
MODELS_DIR = ROOT / "models"
DATASET_CSV = ROOT / "outputs" / "wreck_ml_dataset.csv"

# Load SAR-land sidecar once at import time — safe if file missing
try:
    _SAR_LAND = json.load(open(SAR_LAND_CONFIG, encoding="utf-8"))
except Exception:
    _SAR_LAND = {}

_SAR_PATCH    = _SAR_LAND.get("training_patch", {})
_SAR_RADIUS   = _SAR_LAND.get("detection_radius_m", {})
_SAR_WEIGHTS  = _SAR_LAND.get("scoring_weights", {})
...
def load_known_wrecks() -> list:
    ...
    for wid, w in raw.get("wrecks", {}).items():
        ...
        out.append({
            ...
            "environment": w.get("environment", "water"),  # "land" for aircraft crash sites
            "terrain_type": w.get("terrain_type"),         # alpine / forested_conifer / etc.
        })
    return out


def patch_half_for_wreck(wreck: dict) -> int:
    """Return the correct patch extraction radius for this wreck's environment."""
    if wreck.get("environment") != "land":
        return PATCH_HALF  # default 8 (17x17) for underwater
    aircraft = wreck.get("id", "")
    # Intact fuselage sites get smaller patch; scattered debris fields get larger
    intact_ids = {"beechcraft18_mt_success_nh", "t33_allenspark_co", "cessna414_waterrock_knob_nc"}
    scattered_ids = {"cessna182_humphreys_peak_az"}
    if aircraft in intact_ids:
        return _SAR_PATCH.get("patch_half_intact_fuselage", 10)
    if aircraft in scattered_ids:
        return _SAR_PATCH.get("patch_half_scattered_debris", 20)
    return _SAR_PATCH.get("patch_half_default", 12)


def detection_radius_deg_for_wreck(wreck: dict) -> float:
    """Return the coordinate match radius in degrees for this wreck."""
    if wreck.get("environment") != "land":
        return 0.004  # ~444m default for water wrecks
    radius_m = _SAR_RADIUS.get("default", 200)
    scattered_ids = {"cessna182_humphreys_peak_az", "pa25_cape_lookout_or"}
    if wreck.get("id") in scattered_ids:
        radius_m = _SAR_RADIUS.get("scattered_debris", 400)
    # 1 degree latitude ≈ 111,000m
    return radius_m / 111000.0
...
def extract_patch(data: np.ndarray, row: int, col: int,
                  half: int = PATCH_HALF) -> Optional[np.ndarray]:
    """Extract patch window of (2*half+1) x (2*half+1). Returns None if edge-clipped."""
    ...
    expected = (half*2+1, half*2+1)
    return patch if patch.shape == expected else None
        ...
        with rasterio.open(ref_tif_path) as ref:
            ...
            for wreck in wrecks:
                ...
                if rc is None:
                    continue
                r, c = rc
                wph = patch_half_for_wreck(wreck)  # land wrecks use larger patch

                # Extract patch per band
                patches = {}
                for bname, bd in band_data.items():
                    p = extract_patch(bd, r, c, half=wph)
                    if p is not None:
                        patches[bname] = p
                ...
                feat = {"label": 1, "wreck_id": wreck["id"],
                        ...
                        "environment": wreck.get("environment", "water"),
