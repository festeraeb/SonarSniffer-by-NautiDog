...
def load_wrecks_db() -> dict:
    ...
    Handles v2 schema where wrecks are nested under a 'wrecks' key."""
    if KNOWN_WRECKS_DB.exists():
        raw = json.loads(KNOWN_WRECKS_DB.read_text())
        # v2 schema nests wrecks under "wrecks" key
        if isinstance(raw.get("wrecks"), dict):
            return raw["wrecks"]
        # v1 schema: top-level keys are wreck IDs directly
...
def save_wrecks_db(db: dict):
    """Save wrecks back, preserving v2 envelope if it exists."""
    if KNOWN_WRECKS_DB.exists():
        raw = json.loads(KNOWN_WRECKS_DB.read_text())
        if isinstance(raw.get("wrecks"), dict):
            raw["wrecks"] = db
            KNOWN_WRECKS_DB.write_text(json.dumps(raw, indent=2))
            return
...
def run_probe(wreck_id: str, wreck: dict, probe: dict, zscore: float = 2.5) -> dict:
    ...
    # Build bbox — expand point coords by ~0.02 deg (~2km) if no bbox
    if 'lat_min' in wreck:
        bbox = f"{wreck['lat_min']},{wreck['lon_min']},{wreck['lat_max']},{wreck['lon_max']}"
    elif 'lat' in wreck and wreck['lat'] is not None:
        d = 0.02
        bbox = f"{wreck['lat']-d},{wreck['lon']-d},{wreck['lat']+d},{wreck['lon']+d}"
    else:
        return {"probe": probe["id"], "status": "skipped", "error": "no coordinates"}
...
def probe_all_wrecks(zscore: float = 2.5, probes: list = None) -> dict:
    ...
    for wreck_id, wreck in db.items():
        if not isinstance(wreck, dict) or 'name' not in wreck:
            continue
        print(f"\n{'='*60}")
        ...
        if 'lat_min' in wreck:
            ...
            print(f"  POINT: {wreck['lat']:.4f}, {wreck['lon']:.4f}")
        print(f"{'='*60}")
    ...
    if args.list:
        ...
        for wid, w in db.items():
            if not isinstance(w, dict) or 'name' not in w:
                continue
            tested = ", ".join(w.get("sensors_tested", []) or ["none"])
            print(f"\n  {wid}: {w['name']}")
            if 'lat_min' in w:
                print(f"    BBOX: {w['lat_min']:.2f} to {w['lat_max']:.2f} / "
                      f"{w['lon_min']:.2f} to {w['lon_max']:.2f}")
            elif 'lat' in w and w['lat'] is not None:
                print(f"    POINT: {w['lat']:.4f}, {w['lon']:.4f}")
            print(f"    Depth: {w.get('depth_ft', '?')}ft | Type: {w.get('type', '?')}")
            print(f"    Confidence: {w.get('confidence', '?')}")
            print(f"    Sensors tested: {tested}")
            print(f"    Notes: {w.get('notes', w.get('search_notes', w.get('training_notes', '')))}")
        print()
        return
