...
# Always test CuPy — run directly or imported
import subprocess as _subp
_cupy_probe = _subp.run(
    [sys.executable, '-c',
     'import cupy; t=cupy.array([1,2,3]); print(float(cupy.sum(t)))'],
    capture_output=True, text=True, timeout=10
)
if _cupy_probe.returncode == 0 and '6.0' in _cupy_probe.stdout:
    import cupy as cp
    HAS_GPU = True
    print("  GPU: CuPy enabled (M2200 Quadro)")
else:
    print(f"  GPU: CuPy unavailable — CPU fallback ({_cupy_probe.stderr.strip()[:80]})")
...
def main():
    print("="*80, flush=True)
    print("FULL LAKE MICHIGAN SCAN - M2200 CUDA TO KMZ", flush=True)
    print("="*80, flush=True)
    print(flush=True)
    ...
    print(f"Found {len(tiffs)} TIFFs", flush=True)
    print(f"Processing on M2200 CUDA cores...", flush=True)
    print(flush=True)
    ...
    print(f"PASS 1 — Standard anomaly scan ({len(standard_tiffs)} bands, threshold=band-adaptive)", flush=True)
    print("-"*60, flush=True)
    for i, tiff in enumerate(standard_tiffs, 1):
        print(f"[{i}/{len(standard_tiffs)}] {tiff.name}", flush=True)
