    ...
    for round in 0..max_rounds {
        ...
        if let Some(action_val) = action {
            ...
            if stripped.is_empty() || stripped.len() < 30 {
                // Model only produced thinking, no action or content
                // DIAGNOSTIC: Send the failed exchange to the 8B thinker for analysis
                info!("Think-only response — sending to 8B for diagnosis");
                ...
                if let Some(action_val) = nudged_action {
                    ...
                    if final_response.is_empty() {
                        ...
                        if !final_response.is_empty() {
                            ...
                            final_response = "[No response — model stuck in thinking mode. The 8B diagnosed the issue. Moving to Burn inference will fix this permanently.]".to_string();
                        }
                    }
                    break;
                }
