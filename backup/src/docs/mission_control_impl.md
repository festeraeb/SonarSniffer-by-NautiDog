...
async fn handle_socket(mut socket: WebSocket, state: AppState) {
    tokio::spawn(async move {
        ...
        tokio::spawn(async move {
            ...
            let _ = tx.send(r#"{"type":"llm_token","text":"fn tidal_coeff(lat: f64) -> f64 { 0.5 * lat.sin() }"}"#).await;
            let _ = tx.send(r#"{"type":"result","data":{"status":"success","output":"fn tidal_coeff..."}}"#).await;
        });
