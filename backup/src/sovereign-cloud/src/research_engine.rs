...
impl ResearchDomain {
    ...
    fn arxiv_query(&self) -> &str {
        match self {
            Self::WreckDetection =>
                "shipwreck detection satellite SAR optical remote sensing",
            Self::HydrocarbonRecognition =>
                "oil spill detection SWIR spectral remote sensing ocean",
            Self::TurbidityCompensation =>
                "turbidity correction underwater optical remote sensing bathymetry",
            Self::SearchAndRescue =>
                "search rescue remote sensing drift prediction thermal detection",
            Self::SarProcessing =>
                "SAR speckle coherence change detection maritime",
            Self::Bathymetry =>
                "shallow water bathymetry satellite derived uncertainty",
        }
    }

    fn semantic_scholar_query(&self) -> &str {
        match self {
            Self::WreckDetection => "underwater wreck detection remote sensing",
            Self::HydrocarbonRecognition => "hydrocarbon slick spectral signature satellite",
            Self::TurbidityCompensation => "water turbidity optical correction",
            Self::SearchAndRescue => "maritime search rescue satellite thermal",
            Self::SarProcessing => "synthetic aperture radar maritime detection",
            Self::Bathymetry => "satellite derived bathymetry accuracy",
        }
    }
}
