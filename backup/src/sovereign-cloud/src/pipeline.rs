...
impl PipelineManager {
    ...
    /// If TPU_SERVER_URL is set and this node has no local TPU, the request
    /// is forwarded to the designated TPU server (i7 with Coral).
    async fn pass_scout(&self, req: &TaskRequest) -> Result<PassResult> {
        let start = std::time::Instant::now();
        info!("Pass 1 (Scout) — task {}", req.id);

        // Remote TPU offload: forward to i7 if we don't have local TPU
        if let Ok(tpu_base) = std::env::var("TPU_SERVER_URL") {
            let has_local_tpu = self.allocation.capabilities.read().await.has_tpu;
            if !has_local_tpu {
                info!("Forwarding scout pass to TPU server at {}", tpu_base);
                return self.forward_scout_to_tpu(&tpu_base, req, start).await;
            }
        }

        self.pass_scout_local(req, start).await
    }

    async fn forward_scout_to_tpu(
        &self,
        tpu_base: &str,
        req: &TaskRequest,
        start: std::time::Instant,
    ) -> Result<PassResult> {
        let url = format!("{}/v1/tpu/infer", tpu_base.trim_end_matches('/'));
        ...
        let resp = client
            ...
            .send()
            .await
            .with_context(|| format!("TPU server unreachable at {}", url))?;

        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            warn!("TPU server {} returned {}: {}", url, status, text);
            // Fall through to local execution on TPU server failure
        } else {
            let mut result: PassResult = resp
                .json()
                .await
                .with_context(|| "failed parsing TPU server PassResult")?;
            result.elapsed_ms = start.elapsed().as_millis() as u64;
            info!("Scout (remote TPU) confidence: {:.3} in {}ms", result.anomaly_confidence, result.elapsed_ms);
            return Ok(result);
        }

        // TPU server failed — run locally as fallback
        warn!("TPU server failed — running scout locally");
        self.pass_scout_local(req, start).await
    }

    async fn pass_scout_local(&self, req: &TaskRequest, start: std::time::Instant) -> Result<PassResult> {
        ...
        let tile_id = format!("scout_{:.4}_{:.4}_{}", lat, lon, chrono::Utc::now().timestamp());
        ...
        Ok(PassResult {
            ...
            output: serde_json::json!({
                "tile_id": tile_id, "glint": glint, "hydrocarbon": hydrocarbon,
                "thermal": thermal, "lat": lat, "lon": lon,
            }),
