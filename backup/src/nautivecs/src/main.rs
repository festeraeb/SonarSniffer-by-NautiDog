...
//!   nautivecs-cli serve --port 5003        # Start HTTP API server
...
use nautivecs::{Config, ContextFragment, InjectedContextBuilder, NautivecsEngine};
...
struct Cli {
    ...
    #[arg(long, global = true, default_value = "https://llm.cesarops.org/v1")]
    endpoint: String,
...
enum Commands {
    ...
    /// Start HTTP server (OpenAI-compatible search API)
    Serve {
        /// Port to listen on
        #[arg(short, long, default_value_t = 5003)]
        port: u16,
    },
...
async fn main() -> Result<()> {
    ...
    let mut engine = NautivecsEngine::init(config.clone()).await
    ...
    match cli.command {
        ...
        Commands::Serve { port } => {
            println!("⚓ nautivecs: starting server on port {}...", port);
            nautivecs::serve::run_server(engine, config, port).await?;
        }
