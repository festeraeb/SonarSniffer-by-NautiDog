...
impl MmapGeoTiff {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let mut decoder =
            tiff::decoder::Decoder::new(cursor).map_err(|e| GeoTiffError::Decode(e.to_string()))?;

        let width = decoder
            .dimensions()
            .map_err(|e| GeoTiffError::InvalidTiff(e.to_string()))?
            .0;
        let height = decoder
            .dimensions()
            .map_err(|e| GeoTiffError::InvalidTiff(e.to_string()))?
    ...
    pub fn read_window(
        ...
        let pixels: Vec<u8> = match image {
            ...
            tiff::decoder::DecodingResult::I8(data) => data.iter().map(|v| *v as u8).collect(),
        ...
        for row in 0..y_size {
            for col in 0..x_size {
                for band in 0..self.band_count as u32 {
                    let src_idx = ((y_off + row) * self.width + (x_off + col))
