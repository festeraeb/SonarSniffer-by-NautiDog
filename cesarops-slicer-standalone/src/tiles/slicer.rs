...
pub struct TileManifest {
    ...
    /// Tiles that failed to slice (empty slice, I/O error, etc.).
    /// Failures are logged at WARN level and recorded here instead of
    /// silently disappearing.
    pub failed_tiles: Vec<FailedTileEntry>,
}
...
impl TileSlicer {
    ...
    pub fn slice_all(&self, mission: Option<&MissionSpec>) -> Result<TileManifest> {
        ...
        info!(
            ...
            self.tiff.width, self.tiff.height, tile_cols, tile_rows, self.tile_size, self.tile_size
        );

        // Parallel tile generation — collect Results so failures are visible
        let raw: Vec<(usize, usize, Result<SlicedTile>)> = (0..tile_rows)
            .into_par_iter()
        ...
        let mut failed_tiles: Vec<FailedTileEntry> = Vec::new();
        for (tx, ty, result) in raw {
            match result {
                Ok(tile) => tiles.push(tile),
                Err(e) => {
                    warn!("Tile ({tx},{ty}) failed to slice: {e}");
                    failed_tiles.push(FailedTileEntry {
                        grid_col: tx,
                        grid_row: ty,
                        error: e.to_string(),
                    });
        ...
        for tile in &tiles {
            ...
            fs::write(&pixel_file, &tile.pixels).context("failed to write pixel file")?;
        ...
        if !failed_tiles.is_empty() {
            warn!(
                "{} tile(s) failed to slice and are recorded in manifest.failed_tiles",
                failed_tiles.len()
            );
        }
    ...
    fn slice_tile(
        ...
        debug!("Tile ({tx},{ty}): id={tile_id} delegate={:?}", delegate);
