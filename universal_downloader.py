...
# Push dotenv values to os.environ so earthaccess/boto3/etc can find them
...
class HLSDownloader:
    ...
    def _get_s3_client(self):
        ...
        try:
            ...
            return None
        try:
            resp = self.session.get(self.S3CREDS_URL, timeout=20)
            if resp.status_code != 200:
                print(f"  ⚠ S3 credentials failed: {resp.status_code}")
                return None
            creds = resp.json()
            return boto3.client(
                's3',
