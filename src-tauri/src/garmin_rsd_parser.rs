...
impl ParseResult {
        ...
        if self.pings.is_empty() { return; }
        
        let mut reverse_channels = std::collections::BTreeSet::new();
        let channels: std::collections::BTreeSet<u32> = self.pings.iter().map(|p| p.channel).collect();
        
        for ch in channels {
            let mut starts_with_nadir = 0;
            let mut ends_with_nadir = 0;
            
            let ch_pings: Vec<&Ping> = self.pings.iter().filter(|p| p.channel == ch).take(100).collect();
            if ch_pings.is_empty() { continue; }
        ...
        for ch in &reverse_channels {
            self.healing_actions.push(format!("Reversed samples for channel {} (native nadir at right edge)", ch));
        }
...
impl GarminRSDParser {
    ...
    pub fn parse_file(&mut self, path: &Path) -> ParseResult {
        ...
        healing_actions.push(format!("Generation detected: {} [{}]", generation, gen_signals.join("; ")));
        ...
        while scan_pos + 12 <= bytes.len() {
            ...
            match self.try_parse_record(&bytes, scan_pos, &magic_candidates, generation, &preamble_channels) {
        ...
        for p in &pings {
            let expected_bps: usize = if p.sample_format.contains("i16") { 2 } else { 1 };
            if p.sample_count > 0 {
                let expected_sonar = p.sample_count * expected_bps;
                // Allow ±8 bytes tolerance for varstruct padding/CRC
                if p.sonar_size > 0 && (p.sonar_size as isize - expected_sonar as isize).unsigned_abs() > 8 {
                    sample_mismatches += 1;
                }
            }
        }
    ...
    fn try_parse_record(
        ...
        let channel = if let Some(ch) = channel_from_body {
            ...
            let beam_type = le_u32_padded(body.get(&12).map(Vec::as_slice).unwrap_or(&[])).unwrap_or(0);
            let gen_enum = le_u32_padded(body.get(&6).map(Vec::as_slice).unwrap_or(&[])).unwrap_or(0);
        ...
        let field7_raw = le_u32_padded(body.get(&7).map(Vec::as_slice).unwrap_or(&[])).unwrap_or(0) as usize;
        ...
        let inferred_hint = infer_sample_hint_from_layout(sonar_size, sample_count_raw, sonar.len());
        ...
        let sample_count = normalize_sample_count(sample_count_raw, sonar_size, sonar.len(), sample_hint);
        let latitude = body
            .get(&9)
            .and_then(|b| le_i32(b))
            .map(mapunit_to_deg)
        ...
        let beam_angle_deg = pick_angle_from_fields(&body, &[11, 12, 16], -95.0, 95.0).unwrap_or(0.0);
...
impl GarminRSDParser {
    ...
    pub fn probe_file(&self, path: &Path) -> FileProbe {
        ...
        let summary = if found_any {
            ...
            let hdr_tag  = if found_hdr_crc_ok { "OK" } else { "mismatch (expected — unknown poly)" };
            let body_tag = if found_body_crc_ok { "OK" } else { "sentinel (not a checksum — normal)" };
            format!(
...
fn probe_first_data_record(
    ...
    for _ in 0..20 {
        ...
        for back in 1..=MAX_BACKTRACK_HEADER_START {
            ...
            let f7 = le_u32_padded(body.get(&7).map(Vec::as_slice).unwrap_or(&[]))
...
pub(crate) fn find_next_magic(bytes: &[u8], candidates: &[u32], start: usize, end: usize) -> Option<usize> {
...
fn infer_sample_hint_from_layout(sonar_size: usize, sample_count: usize, sonar_len: usize) -> SampleHint {
    if sample_count == 0 {
    ...
    if (ratio - 2.0).abs() <= 0.25 || (effective as isize - (sample_count * 2) as isize).unsigned_abs() <= 12 {
        return SampleHint::I16;
    }
    if (ratio - 1.0).abs() <= 0.25 || (effective as isize - sample_count as isize).unsigned_abs() <= 12 {
