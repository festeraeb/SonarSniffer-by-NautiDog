...
def cmr_collections_query(keyword: str, max_results: int = 200) -> list:
    """Query CMR collections by keyword."""
    params = {
        "keyword": keyword,
        "has_granules": "true",
    ...
    try:
        ...
        code = e.response.status_code if e.response is not None else "?"
        body = e.response.text[:200] if e.response is not None else ""
        return [{"_error": f"CMR collections HTTP {code}: {body}"}]
    except requests.exceptions.ConnectionError as e:
        return [{"_error": f"CMR collections unreachable: {e}"}]
    except Exception as e:
        return [{"_error": str(e)}]


def discover_landsat_daac_collections(max_collections: int = 200) -> tuple:
    """Return (collections, errors) where collections = [{concept_id, short_name}]."""
    collections, errors = [], []
    seen = set()
    for c in cmr_collections_query("LANDSAT", max_collections):
        if "_error" in c:
            errors.append({"sensor": "Landsat-DAAC", "error": c["_error"]})
            continue

        short_name = (c.get("short_name") or "").upper()
        concept_id = c.get("id") or ""
        if not concept_id or "LANDSAT" not in short_name:
            continue
        if short_name.startswith("HLS"):
            continue
    ...
    collections.sort(key=lambda x: (x.get("short_name", ""), x.get("concept_id", "")))
...
def discover_collections(
    ...
    max_collections: int = 200,
) -> tuple:
    """Return (collections, errors) where collections = [{concept_id, short_name}]."""
    collections, errors = [], []
    seen = set()
    excludes = [x.upper() for x in (exclude_terms or [])]

    for c in cmr_collections_query(keyword, max_collections):
        if "_error" in c:
            errors.append({"sensor": keyword, "error": c["_error"]})
            continue
        ...
        if not concept_id:
            continue
        if include_terms and not any(term.upper() in short_name for term in include_terms):
            continue
        if any(term in short_name for term in excludes):
            continue
        if concept_id in seen:
            continue
        seen.add(concept_id)
...
def _query_landsat_daac(bbox, start, end, max_results) -> tuple:
    granules, errors = [], []
    collections, discover_errors = discover_landsat_daac_collections()
    errors.extend(discover_errors)

    if not collections:
        if not errors:
            errors.append({"sensor": "Landsat-DAAC", "error": "no Landsat collections discovered"})
        return granules, errors
    ...
    for c in collections:
        ...
        short_name = c.get("short_name", "")
        rows = cmr_query(concept_id, bbox, start, end, remaining)
        for g in rows:
            if "_error" in g:
                errors.append({
                    "sensor": "Landsat-DAAC",
...
def _query_discovered_collections(label: str, collections: list, errors: list, bbox, start, end, max_results) -> tuple:
    granules = []
    if not collections:
        if not errors:
            errors.append({"sensor": label, "error": f"no {label} collections discovered"})
        return granules, errors
...
def _query_sentinel3_daac(bbox, start, end, max_results) -> tuple:
    collections, errors = discover_collections(
        keyword="SENTINEL-3",
        include_terms=["SENTINEL-3", "S3"],
        exclude_terms=["HLS"],
    )
    return _query_discovered_collections("Sentinel-3", collections, errors, bbox, start, end, max_results)


def _query_viirs_daac(bbox, start, end, max_results) -> tuple:
    collections, errors = discover_collections(
        keyword="VIIRS",
        include_terms=["VIIRS", "VNP", "VJ1", "VJ2"],
        exclude_terms=[],
    )
    return _query_discovered_collections("VIIRS", collections, errors, bbox, start, end, max_results)
...
def search(bbox: list, start: str, end: str, sensors: list,
    ...
    sensor_map = {
        ...
        "landsat":  lambda: _query_landsat_daac(bbox, start, end, max_results),
        "landsat_daac": lambda: _query_landsat_daac(bbox, start, end, max_results),
        "sentinel3": lambda: _query_sentinel3_daac(bbox, start, end, max_results),
        "sentinel_3": lambda: _query_sentinel3_daac(bbox, start, end, max_results),
        "viirs": lambda: _query_viirs_daac(bbox, start, end, max_results),
...
def main():
    ...
    p.add_argument("--sensor", default="hls",
                   help="comma-sep: hls, sar, landsat, sentinel3, viirs, swot, atl13, modis")
